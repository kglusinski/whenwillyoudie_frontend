...
export default {
  ...
  methods: {
    async calculate(e) {
      ...
      const result = await axios.get(`${process.env.VUE_APP_API_URL}/api/data?age=${this.age}&sex=${this.sex}&place_type=${this.placeType}`, headers).catch(
          () => {
      ...
      if (result.status === 200) {
        ...
        if (this.placeType === 0) {
          const resultRural = await axios.get(`${process.env.VUE_APP_API_URL}/api/data?age=${this.age}&sex=${this.sex}&place_type=1`, headers);
    ...
    async getHistoricalData() {
      ...
      const result = await axios.get(`${process.env.VUE_APP_API_URL}/api/user/data`, headers).catch(
          () => {
            this.$store.commit("logout");
            this.$router.push("/login");
          }
